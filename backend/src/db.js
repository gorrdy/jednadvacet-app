...
// seed placeholder content. Everyone else imports the `db` singleton.
...
function addColumnsIfMissing(table, defs) {
...
// event_rsvp migration: add nullable owner_id so signed-in users can be
// listed by name in the per-event attendee modal (anonymous tokens stay
// anonymous — they just count toward the "+ X dalších" badge).
addColumnsIfMissing("event_rsvp", [
  ["owner_id", "owner_id TEXT"],
]);
db.exec("CREATE INDEX IF NOT EXISTS idx_event_rsvp_owner ON event_rsvp(event_id, status, owner_id)");

// Lightning Address (LUD-16) for chat users. The username is the
// local-part of `<username>@<host>`; backend hosts /.well-known/lnurlp/
// that returns LNURL-pay metadata. Incoming pay requests spawn a
// Cashu mint quote at the user's active mint; the user's app polls
// for pending quotes and claims them as ecash proofs on next open.
addColumnsIfMissing("chat_user", [
...
addColumnsIfMissing("articles", [
  ["source",      "source TEXT NOT NULL DEFAULT 'seed'"],
  ["source_uid",  "source_uid TEXT"],
  ["source_hash", "source_hash TEXT"],
  ["updated_at",  "updated_at TEXT"],
  // Display name of the feed an RSS article came from (e.g. "KryptoHodler").
  // Replaces categories in the Novinky UI — each article shows its source,
  // not a derived topic tag.
  ["feed_name",   "feed_name TEXT"],
]);
db.exec("CREATE INDEX IF NOT EXISTS idx_articles_source ON articles(source)");

// chat_user migration: add avatar + bio columns (profile page feature).
{
...
// channel_message migration: add author_owner_id (Evolu-derived cross-device
// identity) — prod rows pre-dating chat_user have NULL here and fall back to
// the denormalised author_name column.
{
...
// telemetry migration: add user_cookie column (unique-user dedup).
{
...
// admin_user migration: older installs had (email NOT NULL) without a
// username column. Relaxing NOT NULL in SQLite needs a table rebuild.
{
